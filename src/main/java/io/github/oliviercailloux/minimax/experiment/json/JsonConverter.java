...
public class JsonConverter {
   ...
   public static PrintableJsonObject toJson(VoterStrictPreference preference) {
      ...
      if (preferenceBuilder == null) {
         preferenceBuilder = JsonbBuilder.create(
               new JsonbConfig().withAdapters(VoterAdapter.INSTANCE, AlternativeAdapter.INSTANCE).withFormatting(true));
      }
      try {
   ...
   public static VoterStrictPreference toVoterStrictPreference(String json) {
      return JsonbUtils.fromJson(json, VoterStrictPreference.class, VoterAdapter.INSTANCE, AlternativeAdapter.INSTANCE);
   }

   /**
    * TODO call getDelegate() in JsonObjectGeneralWrapper constructor to fail
    * early in case of non convertible.
    */
