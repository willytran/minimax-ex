   ...
   private static PSRWeights genWeights(int nbAlternatives, Supplier<Double> differenceSupplier) {
      ...
      for (double difference : normalizedDifferences.subList(0, normalizedDifferences.size() - 1)) {
         ...
         if (previous < difference) {
            /**
             * Because of imprecision when normalizing, we could end up
             * negative. In this case, we check that this is only the result of
             * a small imprecision error (not some bigger logical error), and
             * round to zero.
             */
            verify(curr > -1e10);
