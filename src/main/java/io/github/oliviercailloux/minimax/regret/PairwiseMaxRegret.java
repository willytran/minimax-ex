...
public class PairwiseMaxRegret {
   ...
   /**
    * Useful when the pmr has been optained by computing, for example, (3 − 2)
    * w1 + (4 − 1) w2, and this differs slightly from (3 w1 + 4 w2) − (2 w1 + 1
    * w2).
    */
   ...
   private PairwiseMaxRegret(Alternative x, Alternative y, Map<Voter, Integer> ranksOfX, Map<Voter, Integer> ranksOfY,
      ...
      checkArgument(
            Math.abs(pmrValue - (getScore(ranksOfY, weights) - getScore(ranksOfX, weights))) < IMPRECISION_TOLERATED);
      if (x.equals(y)) {
   ...
   public String toString() {
      return MoreObjects.toStringHelper(this).add("x", x).add("y", y).add("ranks x", ranksOfX).add("ranks y", ranksOfY)
            .add("weights", weights).add("value", pmrValue).toString();
   }
}
