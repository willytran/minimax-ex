...
//		return sort.get(i);
   ...
   public ImmutableSet<Voter> getQuestionableVoters() {
      return getKnowledge().getVoters().stream().filter(
            v -> getKnowledge().getPartialPreference(v).asTransitiveGraph().edges().size() != getM() * (getM() - 1) / 2)
   ...
   public ImmutableSet<QuestionCommittee> getQuestionsAboutLambdaRangesWiderThanOrAll(double threshold) {
      ...
      if (getM() > 2) {
          ...
          * (equivalently, if all lambda ranges are empty). Considering the way
          * our experiments are currently designed, we assume this has zero
          * probability.
          */
