...
public class StrategyCss implements Strategy {
   ...
   public Question nextQuestion() {
      ...
      final PairwiseMaxRegret pmr = helper.drawFromStrictlyIncreasing(pmrs.asList(), PairwiseMaxRegret.BY_ALTERNATIVES);
      ...
      for (Voter voter : helper.getQuestionableVoters()) {
         ...
         if (!graph.adjacentNodes(xStar).contains(yBar)) {
            ...
            for (Alternative candidate : helper.getKnowledge().getAlternatives()) {
               ...
               /**
                * We give a sufficiently high bonus that candidates that are
                * comparable to exactly one of our currently selected
                * alternatives will be adopted in priority.
                */
               final int bonusScore = comparableToExactlyOne ? helper.getKnowledge().getAlternatives().size() * 2 : 0;
               /**
                * We add each possible questions involving this candidate (that
                * is, one or two questions); preferring to compare to our
                * currently selected alternatives if possible.
                */
               if (!comparableToX) {
               ...
               if (comparableToX && comparableToY) {
                  /**
                   * Lower priority to those cases, we’ll try to select another
      ...
      if (questionsAndScores.isEmpty()) {
         ...
         /**
          * We reach here because for every voter, every alternative is
          * comparable to both x and y. This may happen even if MMR ≠ 0, and
          * irrespective of the profile being completely known. Here is an
          * example found in the wild. Voter1: 1 > 4 > 3 > 2. Voter2: 2 > 1 >
          * {4, 3}. Voter3: 3 > 1 > 4 > 2. Voter4: 2 > 4 > 3 > 1. No constraints
          * on the weights. PMR(x = 1, y = 2) = 1 using weight (1, 0, 0, 0).
          * PMR(x = 2, y = 1) = 1/3 using weights (1, 2/3, 1/3, 0). x* = 2, ybar
          * = 1.
          *
          * Note that questioning the voters further will never help reducing
          * MMR in this example, and I suppose this is always so.
          */
