...
public class MmrLottery {
    ...
    *
    * Thus, the “smallest” question is the best one as we seek to minimize
    * regret.
    *
    */
   ...
   private static ComparatorWithDescription<MmrLottery> getMaxComparator() {
      /**
       * Comparing using pure lexicographic reasoning results in (1.0, 2.0)
       * being considered lower than (0.0, 2.0+1e-16), which we do not want (I
       * have seen a similar situation with max MMRs differing only at the 17th
       * decimal).
       */
      return ComparatorWithDescription.given(Comparator.comparingDouble(l -> l.getWorstMmr() + (l.getBestMmr() / 1e6d)),
            "MAX");
   }

   private static ComparatorWithDescription<MmrLottery> getMinComparator() {
      return ComparatorWithDescription.given(Comparator.comparingDouble(l -> l.getBestMmr() + (l.getWorstMmr() / 1e6d)),
            "MIN");
   }
