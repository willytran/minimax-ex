...
public class UpdateablePreferenceKnowledge implements PreferenceKnowledge {
   ...
   private UpdateablePreferenceKnowledge(Set<Alternative> alternatives, Set<Voter> voters) {
      ...
      if (m == 1) {
         ...
         for (int rank = 1; rank <= m - 2; ++rank) {
            assert m - 2 >= 1;
            /**
             * When d_i/d_{i+1} > n−1, the rule is plurality-i-PD, meaning that
             * when two alternatives share the same number of times they reach
             * rank j, for all j < i, then what counts is the number of times
             * the alternative reaches the rank i, and if ex-æquo, they are
             * resolved using the further weight constraints for lower ranks.
             * Thus, it is unnecessary to distinguish weights greater than n−1.
             *
             * To be more precise, it is useful to distinguish such great
             * differences when giving those weights a cardinal (utilitaristic)
             * interpretation. But this interval seems like a reasonable
             * approximation.
             */
   ...
   public void addConstraint(int rank, ComparisonOperator op, Aprational lambda) {
      ...
      checkArgument(existingRange.isConnected(providedRange), "The provided constraint makes the program infeasible.");
   ...
   /**
    * Check the number of edges in the transitive graphs associated to each
    * voter preference.
    */
   @Override
   ...
   public String toString() {
      return MoreObjects.toStringHelper(this).add("Partial profile", partialProfile).add("Lambda ranges", lambdaRanges)
            .add("Constraints on weights", cow).toString();
   }
